function [F,X,Y] = SpherePotential(XYZ,Q,R,r0,a,b,Dx,Dy,Nxy)
    ...
    for ii = 1:Nxy(1)
        for jj = 1:Nxy(2)
            for ll = 1:n
                if (((r0(1,1) + X(ii, 1) * e1(1,1) + Y(jj, 1) * e2(1,1) - XYZ(ll,1))^2 + (r0(2,1) + X(ii, 1) * e1(2,1) + Y(jj, 1) * e2(2,1) - XYZ(ll,2))^2 + (r0(3,1) + X(ii, 1) * e1(3,1) + Y(jj, 1) * e2(3,1) - XYZ(ll,3))^2)^(0.5) < function [F,X,Y] = SpherePotential(XYZ,Q,R,r0,a,b,Dx,Dy,Nxy)
    n = length(Q);
    e1 = a / ((a(1,1)^2+a(2,1)^2+a(3,1)^2)^0.5);
    c2 = zeros(3,1);
    c2(1, 1) = b(2,1) * a(3,1) - a(2,1) * b(3,1);
